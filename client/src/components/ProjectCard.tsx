...
const ProjectCard = ({
  ...
  const togglePublish = async (projectId: string) =>{
    try {
      ...
      setGeneration((generation)=>generation.map((gen)=>gen.id === projectId ?{...gen, isPublished: data.isPublished}: gen))
  ...
  return (
    <div className="mb-4 break-inside-avoid" key={gen.id}>
        ...
        >
          {gen.generatedImage && (
            <img
              ...
              alt={gen.productName}
              className={`
                absolute inset-0 w-full h-full object-cover transition duration-500 ${gen.generatedVideo ? "group-hover:opacity-0" : "group-hover:scale-105"}`}
            />
          )}
          {gen.generatedVideo && (
            <video
          ...
          {!forCommunity && (
            ...
            >
              ...
              <div className="flex flex-col items-end w-32 text-sm">
                <ul
                  className={`text-xs ${menuOpen ? "block" : "hidden"}  overflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur text-white border border-gray-500/50 rounded-lg shadow-md py-1 mt-2 z-10`}
                >
                  {gen.generatedImage && (
                    <a
                      href="#"
        ...
        <div className="p-4">
          ...
          {!forCommunity && (
            <div className="mt-4 grid grid-cols-2 gap-3">
              <GhostButton onClick={()=>navigate(`/result/${gen.id}`)} className="text-xs justify-center">
                View Details
              </GhostButton>
